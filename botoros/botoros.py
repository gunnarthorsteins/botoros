...
def radio_button(text: str) -> Element:
    return Element(f'input[type="radio"][aria-label="{text} "]')  # The space between the text and the closing quote is essential!
...
add_to_order: Callable[[webdriver.Chrome], None] = partial(
    click, element=Element(by=By.ID, value="menuItemDetail-addItemButton", sleep_duration_s=2)
)
